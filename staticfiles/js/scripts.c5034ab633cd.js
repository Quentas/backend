...
function loaderStarter(me = null){
        let s = 0, e = 5;
        function loader(){
...
function showComments(postID){
    ...
    executeRequest("api/v1/comments/?post_id="+postID, 'GET')
    .then(commentsJSONs => {
                                ...
                                if(usersCounter == [...new Set(users)].length){ // starts after all users have been loaded
                                    commentsJSONs.forEach(commentJSON => {
                                        ...
                                        if(comments.length == commentsJSONs.length){
                                            ...
                                            let m = document.getElementById("commentsBlock_" + postID);
...
function fillPostHTML(type, postJSON, userJSON, me){
    ...
    function getEditHTML(type, me, postJSON){
        if(me == undefined) return '';
            ...
            switch(type){
                case "post": 
                    return (postJSON['user'] == me['id']) ? '<button class="edit_btn" onclick="editPostButton("post", '+postJSON['id']+')">Edit Post</button>' : ''
                case "comment":
                    return (postJSON['user'] == me['id']) ? '<button class="edit_btn" onclick="editPostButton("comment", '+postJSON['id']+')">Edit Comment</button>' : '';
                    default: break;
            }
    }
    switch(type){
        case "post":
            return '<div class="post" id="post_'+postJSON['id']+'">' +
                        ...
                        '<div class="other">' +
                            '<button id="commentsBtn_'+postJSON['id']+'" class="comment_btn" onclick="showComments("post", '+postJSON['id']+')">Comments</button>' +
                            getEditHTML("post", me, postJSON) +
                        ' </div>' +
                        '<div class="commentsBlock" id="commentsBlock_'+postJSON['id']+'">'+
                            '<button class="addComment" onclick="createPostButton("commentToPost",'+postJSON['id']+')">Add Comment</button>'+
                        '</div>' +
                    '</div>';
        case "comment": 
            return '<div class="authorBlock">' +
                        '<img class="authorAvatar" src="'+userJSON['profile_photo']+'" alt="profile photo">'+
                    ...
                    '<div class="otherComment">'+
                        '<button class="otherCommentsButtons" onclick="createPostButton("commentToComment", '+postJSON['id']+')">Reply</button>'+
...
function editPostButton(type, id){
    ...
    if(type=="post") document.getElementById('postText').value = document.getElementById("post_content_"+id).innerHTML;
    else document.getElementById('postText').value = document.getElementById("comment_content_"+id).innerHTML;
}
function updatePost(type, id){
    let postText = document.getElementById('postText');
    switch(type){
        case "commentToPost": executeRequest("api/v1/comments/", 'PUT', JSON.stringify({"id": id, "content": postText.value}), headers); break;
        case "commentToComment": break;
        case "post": executeRequest("api/v1/posts/", 'PUT', JSON.stringify({"id": id, "content": postText.value}), headers); break;
    }
...
function createPost(type, id = null){
    ...
    switch(type){
        case "commentToPost": executeRequest("api/v1/comments/", 'POST', JSON.stringify({"post": id,"content": postText.value}), headers); break;
        case "commentToComment": break;
        case "post": executeRequest("api/v1/posts/", 'POST', JSON.stringify({"content": postText.value}), headers); break;
    }
