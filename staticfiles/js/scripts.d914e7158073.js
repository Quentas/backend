                ...
                if(commentsJSON.length == commentsJSONs.length) // starts after all post have been loaded
                        [...new Set(users)].forEach(user => {
                            ...
                            .then(userJSON => {
                                ...
                                if(usersCounter == [...new Set(users)].length){ // starts after all users have been loaded
                                    commentsJSONs.forEach(commentJSON => {
                                        comments.push({"id" : commentJSON['id'], "postHTML" : fillPostHTML("comment", commentJSON, usersJSON[postJSON['user']], me)});
        ...
        case "post":
            return '<div class="post" id="post_'+postJSON['id']+'">' +
                        ...
                        '<div class="commentsBlock" id="commentsBlock_'+postJSON['id']+'">'+
                            '<button class="addComment" onclick="createPostButton('+postJSON['id']+')">Add Comment</button>'+
                        '</div>' +
                    '</div>';
        case "comment": 
            return '<div class="authorBlock">' +
                    ...
                    '<div class="otherComment">'+
                        '<button class="otherCommentsButtons" onclick="createPostButton("comment", '+postJSON['id']+')">Reply</button>'+
...
function updatePost(type, id){ 
    let url = "";
    if(type == "comment") url = "api/v1/comments/"
    else url  ="api/v1/posts/";
    let postText = document.getElementById('postText');
    executeRequest(url, 'PUT', JSON.stringify({"id": id, "content": postText.value}), 
        {
...
function createPost(type, id = null){
    ...
    if(type == "comment"){
        executeRequest("api/v1/comments/", 'POST', JSON.stringify({"post": id,"content": postText.value}), headers)
    }
    else{
        executeRequest("api/v1/posts/", 'POST', JSON.stringify({"content": postText.value}), 
            {
                "Content-Type" : "application/json",
                "Authorization" : "Token "+window.localStorage.getItem("token")
            })
